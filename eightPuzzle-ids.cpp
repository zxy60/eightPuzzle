...
// 迭代深度的深度优先搜索函数
bool iddfs(const string& initial_state, int empty_pos) {
    for(int depth = 1; depth < 30; depth++){
        stack<BoardState> s;
        set<string> visited;// 设置最大搜索深度
        ...
        while (!s.empty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (nx >= 0 && nx < 3 && ny >= 0 && ny < 3) {//检查空格移动有效性
                    ...
                    if (visited.count(new_state) == 0 && current.depth < depth) {
